import { FC, useState, useEffect } from "react";
import type { Insight } from "@/types";
import { INSIGHT_CONFIG, INSIGHT_REVEAL_INTERVAL_MS } from "@/constants";

interface AIInsightPanelProps {
  insights: Insight[];
}

const AIInsightPanel: FC<AIInsightPanelProps> = ({ insights }) => {
  const visibleCount = useProgressiveReveal(insights.length);

  return (
    <div className="bg-card rounded-lg border border-border overflow-hidden">
      <div className="px-4 py-3 border-b border-border flex items-center gap-2">
        <span className="w-2 h-2 rounded-full bg-primary animate-live-dot" />
        <h3 className="text-sm font-semibold text-foreground">
          Análise IA — Insights ao Vivo
        </h3>
      </div>
      <div className="p-3 space-y-3 max-h-[600px] overflow-y-auto">
        {insights.slice(0, visibleCount).map((insight, i) => (
          <InsightCard key={i} insight={insight} index={i} />
        ))}
      </div>
    </div>
  );
};

// ─── Sub-componente ──────────────────────────────────────────────────

const InsightCard: FC<{ insight: Insight; index: number }> = ({
  insight,
  index,
}) => {
  const config = INSIGHT_CONFIG[insight.type];

  return (
    <div
      className="p-3 rounded-md bg-secondary/40 border border-border/50 animate-fade-up"
      style={{ animationDelay: `${index * 80}ms`, opacity: 0 }}
    >
      <div className="flex items-center gap-2 mb-1.5">
        <span
          className={`text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded ${config.className}`}
        >
          {config.label}
        </span>
        <span className="ml-auto font-mono-data text-[10px] text-muted-foreground">
          {Math.round(insight.confidence * 100)}% conf.
        </span>
      </div>
      <p className="text-xs font-semibold text-foreground mb-0.5">
        {insight.title}
      </p>
      <p className="text-xs text-muted-foreground leading-relaxed">
        {insight.text}
      </p>
    </div>
  );
};

// ─── Hook de revelação progressiva ──────────────────────────────────

function useProgressiveReveal(total: number): number {
  const [count, setCount] = useState(0);

  useEffect(() => {
    if (count >= total) return;

    const timer = setInterval(() => {
      setCount((c) => {
        if (c >= total) {
          clearInterval(timer);
          return c;
        }
        return c + 1;
      });
    }, INSIGHT_REVEAL_INTERVAL_MS);

    return () => clearInterval(timer);
  }, [total, count]);

  return count;
}

export default AIInsightPanel;
